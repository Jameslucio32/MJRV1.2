...
if (isset($_POST['addProduct'])) {
  ...
  if (empty($_POST["prod_code"]) || empty($_POST["prod_name"]) || empty($_POST['prod_desc']) || empty($_POST['prod_price']) || empty($_POST['prod_stock'])) {
    ...
    $prod_stock = $_POST['prod_stock']; // Capture the stock quantity

    // Insert Captured information to a database table
    $postQuery = "INSERT INTO rpos_products (prod_id, prod_code, prod_name, prod_img, prod_desc, prod_price, prod_expiry_date, prod_stock) VALUES(?,?,?,?,?,?,?,?)";
    $postStmt = $mysqli->prepare($postQuery);
    // Bind parameters
    $rc = $postStmt->bind_param('sssssssi', $prod_id, $prod_code, $prod_name, $prod_img, $prod_desc, $prod_price, $prod_expiry_date, $prod_stock);
    $postStmt->execute();
              ...
              <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-row">
                  <div class="col-md-6">
                    ...
                    <input type="number" name="prod_stock" class="form-control" required min="0">
                  </div>
                  <div class="col-md-6">
                    <label>Expiration Date</label>
                    <input type="date" name="prod_expiry_date" class="form-control"> <!-- Removed required attribute -->
                  </div>
                </div>
                <hr>
                <div class="form-row">
