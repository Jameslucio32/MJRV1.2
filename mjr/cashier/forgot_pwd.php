...
function sendOtpEmail($to, $otp) {
    ...
    try {
        ...
        // Recipients
        $mail->setFrom('encoder1295@gmail.com', 'MJR');
...
if (isset($_POST['request_otp'])) {
    if (!filter_var($_POST['reset_email'], FILTER_VALIDATE_EMAIL)) {
        ...
        $checkEmail = mysqli_query($mysqli, "SELECT `staff_email` FROM `rpos_staff` WHERE `staff_email` = '$reset_email'") or exit(mysqli_error($mysqli));
...
if (isset($_POST['confirm_otp'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $update_query = "UPDATE rpos_staff SET password = ? WHERE staff_email = ?";
